...
class GAFM_User( torch.nn.Module ):

    def __init__( self, n_users, n_entitys, k_dim,t_dim,G ):

        super( GAFM_User, self ).__init__( )
    ...
    def gnnForward( self, adj_lists, user_embs ):
        n_hop = 0
        for df in adj_lists:
            if n_hop == 0:
            ...
            if n_hop < len( adj_lists ):
                ...
                aggEmbeddings = self.FMaggregator(entity_embs)
                # [ batch_size, dim ]
                atts = self.attention_item(user_embs, aggEmbeddings)
    ...
    def forward( self, u,i ):
        ...
        users = self.users(u)
        # [batch_size, dim]
        items = self.gnnForward( adj_lists, users )
        # [batch_size]
...
def train(data_set_name,epochs=20,batchSize=1024,k_dim=128,t_dim=64,lr=0.001,need_eva=True):
    ...
    net = GAFM_User(max(user_set)+1,item_graph_pairs.max()+1,k_dim,t_dim,G)

    criterion = torch.nn.BCELoss()
    optimizer = torch.optim.AdamW(net.parameters(), lr=lr,weight_decay=0.5)

    Zcommon.commonTrain(epochs, net, optimizer, criterion,
                        train_set, test_set, batchSize, need_eva, data_set_name, 'GAFMUser')
