...
def train(data_set_name,epochs=10,batchSize=1024,k_dim=128,t_dim=64,lr=0.002,need_eva=True):
    ...
    net = GAFM_Base(max(user_set)+1,item_graph_pairs.max()+1,k_dim,t_dim,G)

    criterion = torch.nn.BCELoss()
    optimizer = torch.optim.AdamW(net.parameters(), lr=lr,weight_decay=0.5)

    Zcommon.commonTrain(epochs, net, optimizer, criterion,
                        train_set, test_set, batchSize, need_eva, data_set_name, 'GAFMBase')
    return net
